import {
  canConfigureBoard as userCanConfigureBoard,
  canCreateTask,
} from "../../board/board.permissions.js";
import { createFilterControls } from "../../features/filters/filter.map.js";
import {
  createNoticeSnackbar,
  createUndoSnackbar,
} from "../../features/feedback/feedback.map.js";
...
export function createBoardPage(state, viewState, actions, workspace) {
  const canConfigureBoard = userCanConfigureBoard(
    state,
  ...
  const configurationButtons = canConfigureBoard
    ? [
        ...
        },
        {
          ...
          type: "button",
          class: "button button--secondary",
          text: "Stages konfigurieren",
          events: { click: actions.openStageConfig },
        },
      ]
    : [];
  /** @type {import("../../core/JaDyDoCo.js").JaDyNode[]} */
    ...
    ? [
        {
          ...
          text: "+ Neue Aufgabe",
          events: { click: () => actions.openCreateTask() },
        },
      ]
    : [];
  return {
            ...
            children: [
              {
                ...
                children: [
                  ...
                  },
                  {
                    ...
                    class: "team-switch",
                    text: "⌄",
                    attrs: { "aria-label": "Team wechseln" },
                  },
                ],
              },
              {
                ...
                children: [
                  {
                    ...
                    children: [
                      ...
                      },
                      {
                        ...
                        class: "sidebar__add",
                        text: "+",
                        attrs: { "aria-label": "Board hinzufügen" },
                        events: { click: actions.openCreateBoard },
                      },
                    ],
                  },
                  ...workspace.boards.map((board, index) =>
                    boardLink(board, workspace.activeBoardId, index, actions),
                  ),
                ],
              },
              {
                ...
                children: [
                  sidebarLink("?", "Hilfe & Feedback", "#help"),
                  {
                    tagName: "button",
                    type: "button",
                    class: "sidebar-link sidebar-settings",
                    events: { click: actions.openAppSettings },
                    children: [
                      {
                        tagName: "span",
          ...
          },
          {
            ...
            children: [
              ...(workspace.persistenceError
                ? [createPersistenceWarning(actions)]
                : []),
              {
                tagName: "header",
                class: "board-header",
                children: [
                  ...
                  },
                  {
                    ...
                    children: [...configurationButtons, ...taskCreationButtons],
                  },
                ],
              },
              {
      ...
      ...(viewState.selectedTaskId && state.tasks[viewState.selectedTaskId]
        ? [
            createTaskDetails(
              state,
              actions,
              state.tasks[viewState.selectedTaskId],
              workspace.users,
              workspace.activeUserId,
              viewState.taskEditOpen,
            ),
          ]
        : []),
      ...(canConfigureBoard && viewState.boardConfigOpen
        ? [
            createBoardConfig(
              state,
              actions,
        ...
        : []),
      ...(canConfigureBoard && viewState.stageConfigOpen
        ? [createStageConfig(state, viewState, actions)]
        : []),
      ...(canConfigureBoard &&
      viewState.stageEditor &&
      !viewState.stageConfigOpen
        ? [createStageEditorDialog(state, viewState, actions)]
        : []),
      ...(workspace.createOpen ? [createBoardDialog(actions)] : []),
      ...(workspace.userSettingsOpen
        ? [createUserSettings(workspace, actions)]
        : []),
      ...(workspace.appSettingsOpen
        ? [createAppSettings(workspace, actions)]
        : []),
    ],
  };
}
...
function boardMemberStack(project, users, actions, canConfigure) {
  ...
  const memberIds = [
    ...new Set(project.memberIds.filter((id) => id !== project.ownerId)),
  ];
  ...
  if (remaining)
    avatars.push({
      ...
      text: `+${remaining}`,
      attrs: { title: `${remaining} weitere Board-Mitglieder` },
    });
  /** @type {import("../../core/JaDyDoCo.js").JaDyNode[]} */
  const children = [
    {
      ...
      children: [
        {
          tagName: "span",
          class: "member board-owner__avatar",
          text: owner?.initials ?? "?",
          attrs: { title: `${owner?.name ?? "Unbekannt"} · Board-Owner` },
        },
      ],
    },
  ];
  ...
  if (canConfigure) {
    return {
      ...
      class: "member-stack",
      attrs: {
        "aria-label": `Board-Owner und ${memberIds.length} Mitglieder verwalten`,
      },
      events: { click: actions.openBoardConfig },
      children,
    };
...
function boardLink(board, activeBoardId, index, actions) {
  ...
  return {
    ...
    class: [
      "project-link",
      "board-link",
      board.id === activeBoardId && "board-link--active",
    ],
