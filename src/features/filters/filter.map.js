...
/** @typedef {{ setFilter: (key: "query"|"priority"|"category"|"assignee", value: string) => void, clearFilters: () => void }} FilterActions */
 ...
 */
export function createFilterControls(state, viewState, actions) {
  const categories = uniqueTaskValues(state, "category");
  const assignees = uniqueTaskValues(state, "assignee");
  ...
  const assigneeCounts = countTasksByFacet(state, "assignee", viewState.filters);
  return [
    ...
    filterSelect("assignee", "Verantwortlich", [facetOption("all", "Alle Personen", assigneeCounts, viewState.filters.assignee), ...assignees.map((value) => facetOption(value, value, assigneeCounts, viewState.filters.assignee))], viewState.filters.assignee, actions),
...
/** @param {"priority"|"category"|"assignee"} name @param {string} label @param {import("../../core/JaDyDoCo.js").SelectOption[]} options @param {string} selectedValue @param {FilterActions} actions @returns {import("../../core/JaDyDoCo.js").JaDyNode} */
...
/** @param {import("../../board/board.state.js").BoardState} state @param {"category"|"assignee"} key */
