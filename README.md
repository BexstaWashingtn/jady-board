...
Der Client lädt dann die zugänglichen Boards aus PostgreSQL. Neue Tasks sowie Änderungen an Titel, Kategorie, Priorität, Fälligkeit, Status, Zuweisung und Todos bestehender Tasks werden bereits an den Server geschrieben. Andere Änderungen sind in diesem Zwischenstand nur im Arbeitsspeicher sichtbar; ein Neuladen stellt dafür den Datenbankstand wieder her. Ist die API beim Start nicht erreichbar, verwendet der Client weiterhin den lokalen Workspace.
